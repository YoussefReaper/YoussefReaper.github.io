...
async function loadSettings() {
    try {
        ...
        // Ensure all color properties exist (merge with defaults)
...
function applyColorPreset(preset) {
    // Check if coin system is available and if preset is unlocked
    if (window.coinSystem && !window.coinSystem.hasUnlocked('colorPresets', preset)) {
...
function loadPurchasedTopbarThemes() {
    ...
    // Get purchased items from localStorage
    const purchasedItems = JSON.parse(localStorage.getItem('purchasedItems') || '{}');
    const topbarThemes = purchasedItems.topbarThemes || [];
...
window.goToShop = function(itemId) {
    const shopUrl = `shop.html#${itemId}`;
