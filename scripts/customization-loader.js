...
(function() {
    ...
    if (window.customizationLoaderInitialized) {
        console.warn('Customization loader already initialized, skipping...');
        return;
    }
    window.customizationLoaderInitialized = true;
    ...
    async function loadAndApplySettings() {
        ...
        if (window.customizationLoading) {
            console.warn('Customization already loading, skipping duplicate call...');
            return;
        }
        window.customizationLoading = true;
        try {
            ...
            if (saved) {
                ...
                if (hasUserCustomizations) {
                    // User has customizations - don't merge with defaults
                    currentSettings = savedSettings;
                    console.log('Loader: Loading user customizations (no default merge)');
                } else {
                    // First time or no custom flag - merge with defaults
                    currentSettings = { ...DEFAULT_SETTINGS, ...savedSettings };
                    console.log('Loader: Loading settings with default merge (first time)');
                    
                    // Mark that user now has customizations to prevent future default merging
                    localStorage.setItem('hasUserCustomizations', 'true');
                }
            } else {
                // No saved settings at all - use defaults only for first time
                if (!hasUserCustomizations) {
                    currentSettings = { ...DEFAULT_SETTINGS };
                    console.log('Loader: Using default settings (first time)');
                } else {
                    // User had customizations but localStorage was cleared - don't apply defaults
                    console.log('Loader: No saved settings but user had customizations - keeping current');
                    return;
                }
            }
            ...
            // Don't reset to defaults if user has customizations
            const hasUserCustomizations = localStorage.getItem('hasUserCustomizations') === 'true' ||
                                         localStorage.getItem('preventDefaultReversion') === 'true';
            if (!hasUserCustomizations) {
                currentSettings = { ...DEFAULT_SETTINGS };
        ...
        window.customizationLoading = false;
    }
    ...
    function initialize() {
        // Additional protection against multiple initialization
        if (window.customizationLoaderRunning) {
            console.warn('Customization loader already running, skipping...');
            return;
        }
        window.customizationLoaderRunning = true;
