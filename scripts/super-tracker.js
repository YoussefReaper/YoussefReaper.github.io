...
class EnhancedTaskTracker {
    constructor() {
        ...
        this.actualStartTime = null; // Track the very first start time
    ...
    init() {
        ...
        // Ensure initial mode is set correctly and button is enabled for non-task modes
        console.log(`🚀 Enhanced Task Tracker initialized with mode: ${this.currentMode}`);
        
        // Immediately set button state for current mode
        this.updateModeSpecificUI();
        
        // Force initial UI update for button state
        setTimeout(() => {
            this.updateModeSpecificUI();
            console.log('🔄 Initial mode-specific UI update completed');
            
            // Force another update to ensure button state is correct
            setTimeout(() => {
                this.updateModeSpecificUI();
                console.log('🔄 Secondary mode-specific UI update completed');
            }, 100);
        }, 200);
    }
    ...
    updateTaskDisplay() {
        ...
        // Update button state based on current mode (don't force enable)
        this.updateModeSpecificUI();
    }
    ...
    startTimer() {
        console.log(`🎯 startTimer() called - isPaused: ${this.isPaused}, isRunning: ${this.isRunning}, elapsedTime: ${this.elapsedTime}ms`);
        ...
        // For pomodoro and stopwatch modes, we don't require a task
        ...
        if (!this.actualStartTime) {
            ...
            console.log('🎯 Fresh start - resetting all time tracking');
            ...
            // This should never happen since we have resumeTimer() for resumes
            console.log('🎯 WARNING: startTimer called when actualStartTime exists - this might be an error');
        }
        
        console.log(`🎯 Timer started fresh - startTime: ${new Date(this.startTime).toLocaleTimeString()}`);
        ...
        switch (this.currentMode) {
            case 'task-focused':
                ...
                break;
            case 'pomodoro':
                message = 'Started 25-minute Pomodoro session' + (this.selectedTask ? ` for: ${this.selectedTask.title}` : '');
                break;
            case 'stopwatch':
                message = 'Stopwatch started - earn 20 coins per hour!' + (this.selectedTask ? ` Tracking: ${this.selectedTask.title}` : '');
                break;
            case 'custom':
                const customMinutes = this.getCustomTime() / 60;
                message = `Started ${customMinutes}-minute custom timer` + (this.selectedTask ? ` for: ${this.selectedTask.title}` : '');
                break;
            default:
    ...
    pauseTimer() {
        ...
        // Calculate current elapsed time (total runtime minus paused time)
        this.elapsedTime = now - this.actualStartTime - this.totalPausedTime;
        this.pauseStartTime = now;
        
        console.log(`⏸️ Pausing at: ${this.formatTime(this.elapsedTime)} (${this.elapsedTime}ms)`);
        console.log(`⏸️ Total paused time so far: ${this.totalPausedTime}ms`);
        console.log(`⏸️ Actual start time: ${new Date(this.actualStartTime).toLocaleTimeString()}`);
        
        this.isRunning = false;
        this.isPaused = true;
        
        // Save current progress for task-focused mode
        if (this.currentMode === 'task-focused') {
        ...
        console.log('⏸️ Timer paused - State saved');
        this.showNotification('Timer paused - Page will refresh', 'info');
        
        // Refresh the page after a short delay to allow notification to show
        setTimeout(() => {
            console.log('🔄 Refreshing page due to pause');
    ...
    // Resume timer
    resumeTimer() {
        if (this.isRunning || !this.isPaused || !this.pauseStartTime) return;
        ...
        console.log(`▶️ Resuming from: ${this.formatTime(this.elapsedTime)} (${this.elapsedTime}ms)`);
        console.log(`▶️ Was paused for: ${pauseDuration}ms`);
        console.log(`▶️ Total paused time: ${this.totalPausedTime}ms`);
        ...
        console.log('▶️ Timer resumed - State restored');
    ...
    updateTimer() {
        if (!this.isRunning || !this.actualStartTime) return;
        
        const now = Date.now();
        
        // Calculate elapsed time as: (current time - actual start time - total paused time)
        const previousElapsed = this.elapsedTime;
        this.elapsedTime = now - this.actualStartTime - this.totalPausedTime;
        this.sessionElapsedTime = now - this.sessionStartTime;
        
        // Debug: log if there's an unexpected time change
        const timeDiff = Math.abs(this.elapsedTime - previousElapsed);
        if (timeDiff > 2000) { // More than 2 seconds difference
            console.log(`⚠️ Unexpected time jump: ${previousElapsed}ms -> ${this.elapsedTime}ms (diff: ${timeDiff}ms)`);
            console.log(`   Actual start: ${new Date(this.actualStartTime).toLocaleTimeString()}`);
            console.log(`   Total paused: ${this.totalPausedTime}ms`);
    ...
    saveCurrentProgress() {
        if (!this.selectedTask || this.elapsedTime === 0) return;
        
        const progress = this.getTaskProgress(this.selectedTask.id);
        progress.timeSpent = (progress.timeSpent || 0) + this.elapsedTime;
    ...
    resetTimer() {
        ...
        console.log('🔄 Timer reset - All state cleared');
    }
    ...
    updateControlButtons() {
        ...
        if (this.isRunning) {
            ...
            // Don't override disabled state when paused
            pauseBtn.style.display = 'none';
        } else {
            startBtn.style.display = 'inline-flex';
            startBtn.innerHTML = '<i class="fas fa-play"></i><span>Start</span>';
            // Don't override disabled state - let updateModeSpecificUI handle it
            pauseBtn.style.display = 'none';
        }
    ...
    handleGlobalKeyboard(event) {
        ...
        // Check if maximize mode is active
        const maximizeOverlay = document.getElementById('maximize-overlay');
        const isMaximizeMode = maximizeOverlay && maximizeOverlay.style.display !== 'none';
        
        // Only handle spacebar for timer control in normal mode
        if (!isMaximizeMode && event.key === ' ') {
            event.preventDefault();
            
            // Check if start button is disabled (for task-focused mode without task)
            const startBtn = document.getElementById('start-btn');
    ...
    setTimerMode(mode) {
        ...
        if (selectedOption) {
            ...
            console.log(`✅ Mode option activated for ${mode}`);
        }
        ...
        // Always show task selection, but make it optional for pomodoro and stopwatch
        if (taskSelection) {
            taskSelection.style.display = 'block';
            
            // Update task selection header based on mode
            const taskHeader = taskSelection.querySelector('h4');
            if (taskHeader) {
                switch (mode) {
                    case 'task-focused':
                        taskHeader.innerHTML = '<i class="fas fa-list"></i> Select Task (Required)';
                        break;
                    case 'pomodoro':
                        taskHeader.innerHTML = '<i class="fas fa-list"></i> Link to Task (Optional)';
                        break;
                    case 'stopwatch':
                        taskHeader.innerHTML = '<i class="fas fa-list"></i> Track Task (Optional)';
                        break;
                    case 'custom':
                        taskHeader.innerHTML = '<i class="fas fa-list"></i> Link to Task (Optional)';
                        break;
        ...
        // Force UI update to ensure everything is in sync
        setTimeout(() => {
            this.updateModeSpecificUI();
        }, 100);
    ...
    updateModeSpecificUI() {
        ...
        console.log(`🔧 updateModeSpecificUI called for mode: ${this.currentMode}`);
        ...
        if (startBtn) {
            switch (this.currentMode) {
                case 'task-focused':
                    ...
                    console.log(`🎯 Task-focused mode: button disabled = ${!this.selectedTask}`);
                    if (timerDescription) {
                ...
                case 'pomodoro':
                    ...
                    console.log(`🍅 Pomodoro mode: button enabled`);
                    if (timerDescription) {
                ...
                case 'stopwatch':
                    startBtn.disabled = false;
                    console.log(`⏱️ Stopwatch mode: button enabled`);
                    if (timerDescription) {
                ...
                case 'custom':
                    startBtn.disabled = false;
                    console.log(`⚙️ Custom mode: button enabled`);
            ...
            console.log(`🎯 Final button state: disabled = ${startBtn.disabled}`);
        } else {
    ...
    updateUI() {
        this.updateControlButtons();
        this.updateModeSpecificUI(); // Ensure button state is correct for current mode
        this.updateProgressStats();
        ...
        if (sessionElement) {
            if (this.selectedTask) {
                ...
                sessionElement.textContent = this.currentMode === 'task-focused' ? 'Select a task to begin' : 'Ready to start';
            }
        }
    }
        ...
        if (sessionProgress) {
            switch (this.currentMode) {
                case 'pomodoro':
                    ...
                    break;
                case 'stopwatch':
                    sessionProgress.textContent = 'Earn 20 coins per hour';
                    break;
                case 'custom':
                    const customTime = this.getCustomTime();
                    sessionProgress.textContent = `${customTime} minute custom session`;
                    break;
                case 'task-focused':
                default:
    ...
    // Update maximize timer display
    updateMaximizeTimer() {
        const maximizeTimer = document.getElementById('maximize-timer-display');
        const maximizeTimer2 = document.getElementById('maximize-timer'); // Alternative ID
        const maximizeModeIndicator = document.getElementById('maximize-mode-indicator');
        
        // Sync timer display - use the same logic as main timer
        let displayTime;
        ...
        // Update both possible timer displays
        if (maximizeTimer) {
            maximizeTimer.textContent = displayTime;
        }
        if (maximizeTimer2) {
            maximizeTimer2.textContent = displayTime;
        }
        
        // Update mode indicator
        if (maximizeModeIndicator) {
            switch (this.currentMode) {
                case 'pomodoro':
                    maximizeModeIndicator.textContent = 'Pomodoro Session';
                    break;
                case 'stopwatch':
                    maximizeModeIndicator.textContent = 'Stopwatch Mode';
                    break;
                case 'custom':
                    maximizeModeIndicator.textContent = 'Custom Timer';
                    break;
                case 'task-focused':
                default:
                    if (this.selectedTask) {
                        maximizeModeIndicator.textContent = this.selectedTask.title;
                    } else {
                        maximizeModeIndicator.textContent = 'Select a task';
                    }
                    break;
            }
        }
        
        // Update maximize progress rings to match main rings
